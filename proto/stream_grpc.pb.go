...
func (c *streamServiceClient) Stream(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[StreamPayload, StreamResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &StreamService_ServiceDesc.Streams[0], StreamService_Stream_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[StreamPayload, StreamResponse]{ClientStream: stream}
	return x, nil
}
...
func (UnimplementedStreamServiceServer) Stream(grpc.BidiStreamingServer[StreamPayload, StreamResponse]) error {
...
func _StreamService_Stream_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(StreamServiceServer).Stream(&grpc.GenericServerStream[StreamPayload, StreamResponse]{ServerStream: stream})
}
